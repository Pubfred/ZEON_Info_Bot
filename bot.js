...
class ExchangeData {
    ...
    fill(price, volume, buy, sell, change) {
        ...
        this.volume = isNaN(volume) ? undefined : parseFloat(volume).toFixed(0);
    ...
    return new Promise((resolve, reject) => {
	    ...
	    case "cratex": {
            ...
            case "zeonexchange": {
                ...
                js_request(`https://exchange.zeonhexalgo.fun/page/api?method=singlemarket&marketid=53` , res => exdata.fillj(res["return"][0], "lasttradeprice", "volume24h", "sell", "buy", ""));
                break;
            }
        ...
        return Promise.all([
            ...
            for (let stat of conf.statorder) {
                switch (stat) {
                    ...
                    case "mncount": {
                        if (valid.mncount1)
                            embed.addField("MN Count Light", mncount1.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), true);
                        break;
                    }
                    case "supply": { 
                        if (valid.supply)
        ...
        return Promise.all([
            ...
            for (let stat of conf.statorder) {
                switch (stat) {
                    ...
                    case "mncount": {
                        if (valid.mncount2)
                            embed.addField("MN Count Medium", mncount2.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), true);
                        break;
                    }
                    case "supply": {
                        if (valid.supply)
        ...
        return Promise.all([
            ...
            for (let stat of conf.statorder) {
                switch (stat) {
                    ...
                    case "mncount": {
                        if (valid.mncount3)
                            embed.addField("MN Count Full", mncount3.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), true);
                        break;
                    }
                    case "supply": {
                        if (valid.supply)
